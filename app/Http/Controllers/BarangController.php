...
'<button type="submit" class="btn btn-danger btn-sm" onclick="return confirm(\'Apakah Kita yakit menghapus data ini?\');">Hapus</button></form>';*/
    ...
    public function export_excel()
    {
        ...
        foreach (range('A', 'F') as $columnID) {
            $sheet->getColumnDimension($columnID)->setAutoSize(true); //auto size column
        }

        $sheet->setTitle('Data Barang'); //set title sheet

        $writer = IOFactory::createWriter($spreadsheet, 'Xlsx'); //create writer
        $filename = 'Data_Barang' . date('Y-m-d H:i:s') . '.xlsx'; //set filename
        ...
        $writer->save('php://output'); //save to output
        exit;
